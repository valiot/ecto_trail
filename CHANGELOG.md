...
- `log/*` and `*_and_log/*` no longer raise `Ecto.ConstraintError` (e.g. duplicate `audit_logs_pkey`) on audit insert; failures are logged and swallowed so the caller's transaction succeeds. Closes OPS-4581.
