...
- `log_changes/5` and `log_changes_alone/6` (used by `*_and_log`) now use `on_conflict: :nothing` + `unique_constraint` on audit_log pkey to avoid Ecto.ConstraintError on duplicate key (retries/double-log inside tx). Closes OPS-4606.
