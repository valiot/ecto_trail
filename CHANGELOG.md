...
- `changelog_changeset` now declares `unique_constraint(:id, name: ...)` for the audit_log pkey, preventing Ecto.ConstraintError during `*_and_log` and `log` paths when the database returns a pkey unique violation. Closes OPS-4589.
