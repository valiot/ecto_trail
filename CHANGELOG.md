...
- Declare `unique_constraint` on the audit-log table pkey in `changelog_changeset`; `*_and_log` and `log` calls inside transactions no longer raise unhandled `Ecto.ConstraintError` on pkey collisions (e.g. sequence rewind). Closes OPS-4574.
