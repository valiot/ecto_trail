...
- Declare `unique_constraint/3` on the (configurable) audit log pkey so `*_and_log/*` and `log/*` convert `Ecto.ConstraintError` ("<table>_pkey") into a changeset error instead of raising inside the caller's transaction. Closes OPS-4585.
