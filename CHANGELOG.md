...
- `log_changes` / `*_and_log` now declare `unique_constraint/3` on audit_log pkey (via table_name) so duplicate-key races yield changeset errors instead of Ecto.ConstraintError. Closes OPS-4623.
