defmodule EctoTrailTest do
  ...
  describe "audit log pkey constraint handling (OPS-4590)" do
    setup do
      {:ok, schema} = TestRepo.insert(%Resource{name: "name"})
      {:ok, %{schema: schema}}
    end

    test "update_and_log succeeds and does not raise Ecto.ConstraintError when audit log insert hits pkey unique violation (sequence skew or id collision)",
         %{schema: schema} do
      alias Ecto.Changeset
      alias EctoTrail.Changelog

      # Seed a changelog row with a specific high id, then rewind the sequence
      # so the *next* autogenerated id will collide on the pkey.
      seed_id = 2_000_000_123
      ...
      seed_cs =
        %Changelog{}
        |> Changeset.cast(seed_attrs, [:id, :actor_id, :resource, :resource_id, :changeset, :change_type])

      TestRepo.insert!(seed_cs)

      # Rewind the sequence for the audit_log table so the next insert attempts the colliding id.
      seq_name = "audit_log_id_seq"
      Ecto.Adapters.SQL.query!(TestRepo, "SELECT setval($1::regclass, $2, true);", [seq_name, seed_id])

      # This update_and_log will internally call log_changes -> repo.insert for the Changelog.
      # Without unique_constraint on the pkey in changelog_changeset, this raises Ecto.ConstraintError.
      # With the fix, the violation is turned into a changeset error, swallowed by log_changes
      # (existing soft-fail behavior), and the main resource update commits.
      result =
        schema
      ...
      # No audit log entry for this actor's update should exist (the log insert hit the conflict and was treated as soft failure).
      refute TestRepo.exists?(
               from(c in Changelog,
                 where: c.actor_id == "pkey-collision-actor" and c.change_type == :update
               )
             )
    end
  end
